...
<html>
	...
	<body>
			...
			<div class="col-xs-12 col-md-11 col-md-offset-1">
				...
				<div class="panel panel-primary">
					...
					<div class="panel-body">
						...
						<form class="form-inline" action="<%=basePath %>secretary/findApplyTitle.shtm" method="post" id="select">
							...
							<input type="hidden" value="${status}" id="statusHid">
		...
		<div class="row" id="row2">
			<div class="col-xs-12 col-md-11 col-md-offset-1">
				<form method="post" action="<%=basePath %>secretary/findApplyTitle_checkStuTitile.shtm">
					<div class="panel panel-primary">
						<div class="panel-heading">
							<span class="fa fa-check-square-o"></span>&nbsp;&nbsp;学生自拟课题
						</div>
							...
							<table style="text-align:center" class="table table-bordered table-hover">
								...
								<tbody>
									...
									<tr>
										...
										<td>${applyTitle.stuMajor}</td>
										<td>${applyTitle.stuName}</td>

										<td>${applyTitle.stuNum}</td>
										<td>${applyTitle.title}</td>
										<td>${applyTitle.teaName}</td>
										<td>
											<c:if test="${applyTitle.managerStatus==0}">
												待审核
												</c:if>
												<c:if test="${applyTitle.managerStatus==1}">
												已通过
												</c:if>
												<c:if test="${applyTitle.managerStatus==2}">
												未通过
											</c:if>
										</td>
										<td>
											<a href="<%=basePath %>secretary/findApplyTitle_findStuTitleInfo.shtm?id=${applyTitle.id}">审核</a>
										</td>
									</tr>
									</c:forEach>
						...
						<div class="panel-footer">						
							...
							<div class="col-xs-8 col-md-7" style="float: right; text-align:right;">
