...
<body>
			...
			<div class="col-md-9 col-md-offset-1">
				<div class="panel panel-primary">
					<div class="panel-heading">
						<span class="fa fa-edit"></span>&nbsp;&nbsp;开题报告：
						<c:choose>
						<c:when test="${openingReport==null}">未提交</c:when>
						<c:when test="${openingReport.teaStatus==2||openingReport.leaderStatus==2}">退回</c:when>
						<c:when test="${openingReport.teaStatus==1&&openingReport.leaderStatus==1}">通过</c:when>
						<c:otherwise>审核中</c:otherwise>
						</c:choose>		
					</div>
					<form method="post" enctype="multipart/form-data" action="<%=basePath%>student/openingReport_submitReport.shtm">
						...
						<table
							...
							<c:if test="${openingReport!=null}">
								<tr>
									...
									<td><c:if test="${openingReport.leaderStatus==0}">审核中</c:if>
										<c:if test="${openingReport.leaderStatus==1}">通过</c:if>
										<c:if test="${openingReport.leaderStatus==2}">退回</c:if>
									</td>
									<td><a id="cancelOpeningReport" href="javascript:void(0);">撤回</a></td>
								</tr>
							</c:if>
							</tbody>
						</table>
		...
		<div class="modal-content">
			...
			<div class="modal-footer">
				<a type="submit" id="unApplySubmit" class="btn btn-primary"
					href="<%=basePath%>student/openingReport_cancel.shtm?id=${openingReport.id}">确认</a>
				<button type="button" class="btn btn-default"
					data-dismiss="modal">取消</button>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		$(function(){
			$("#submitOpeningReport").click(function(){
		...
		$(function(){
			$("#cancelOpeningReport").click(function(){
				$(".modal-title").html("撤回提示");
				$(".modal-body").html("仅限撤回审核中的开题报告，确认撤回？");
