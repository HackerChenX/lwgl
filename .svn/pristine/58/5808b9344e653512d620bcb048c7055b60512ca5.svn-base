...
<body>
			...
			<div class="col-md-9 col-md-offset-1">
				<div class="panel panel-primary">
					<div class="panel-heading">
						<span class="fa fa-edit"></span>&nbsp;&nbsp;论文定稿：
						<c:choose>
						<c:when test="${finalPaper==null}">未提交</c:when>
						<c:when test="${finalPaper.teaStatus==2}">退回</c:when>
						<c:when test="${finalPaper.teaStatus==1}">通过</c:when>
						<c:otherwise>审核中</c:otherwise>
						</c:choose>	
					</div>
					<form method="post" enctype="multipart/form-data" action="<%=basePath%>student/finalPaper_submitFinalPaper.shtm">
						<div class="panel-body">
							...
							<div class="alert alert-info" role="alert"
								...
								<p>（1）选择附件点击提交后上传</p>
								<p>（2）仅限撤回审核中的文件</p>
								<p>（3）附件上传后由系统统一命名为:论文定稿-姓名-学号格式</p>
							</div>
							<div>
					...
					<div class="panel-body">
						<table
							...
							<c:if test="${finalPaper!=null}">
								<tr>
									<td>${finalPaper.createUser}</td>
									<td><a href="<%=basePath%>student/downloadFile.shtm?filePath=${finalPaper.finalPaperPath}">许昌学院本科毕业论文（设计）论文定稿</a></td>
									<td><c:if test="${finalPaper.teaStatus==0}">审核中</c:if>
										<c:if test="${finalPaper.teaStatus==1}">通过</c:if>
										<c:if test="${finalPaper.teaStatus==2}">退回</c:if>
									</td>							
									<td><a id="cancelFinalPaper" href="javascript:void(0);">撤回</a></td>
								</tr>
							</c:if>
							</tbody>
						</table>
		...
		<div class="modal-content">
			...
			<div class="modal-footer">
				<a type="submit" id="unApplySubmit" class="btn btn-primary"
					href="<%=basePath%>student/finalPaper_cancel.shtm?id=${finalPaper.id}">确认</a>
				<button type="button" class="btn btn-default"
					data-dismiss="modal">取消</button>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		$(function(){
			$("#submitFinalPaper").click(function(){
		...
		$(function(){
			$("#cancelFinalPaper").click(function(){
				$(".modal-title").html("撤回提示");
				$(".modal-body").html("仅限撤回审核中的论文定稿，确认撤回？");
